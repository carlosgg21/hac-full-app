...
<div class="mb-6">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">New Contact</h1>
            <p class="text-sm text-gray-500 mt-1">Add a new client or contact</p>
        </div>
...
<form method="POST" action="<?= Response::url('/clients') ?>" id="clientCreateForm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="state" id="state"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition"
                                   value="<?= htmlspecialchars($post['state'] ?? '') ?>"
                                   placeholder="State">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="zip_code" id="zip_code"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition"
                                   value="<?= htmlspecialchars($post['zip_code'] ?? '') ?>"
                        ...
                        <div>
                            ...
                            <input type="text" name="country" id="country"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition"
                                   value="<?= htmlspecialchars($post['country'] ?? 'México') ?>"
                                   placeholder="Country">
                        </div>
                    </div>
                </div>
            ...
            <section class="bg-white rounded-xl shadow-md px-6 py-5">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition"
                              placeholder="Optional notes about this contact"><?= htmlspecialchars($post['notes'] ?? '') ?></textarea>
                </div>
            </section>
        </div>
    </div>
